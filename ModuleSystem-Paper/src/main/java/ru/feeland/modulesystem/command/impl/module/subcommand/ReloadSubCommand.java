...
public class ReloadSubCommand extends BaseSubCommand {
    public ReloadSubCommand(BaseModuleSystem plugin) {
        super(plugin);
    }
    //<editor-fold desc="CommandMethods" defaultstate="collapsed">
    private void sendReloadResult(CommandDTO dto, BaseMessagesConfig messages, ModuleOperationResult result) {
        ...
        if (arg.equals("all")) {
            ...
            for (ModuleInfo info : moduleService.getModuleInfoList()) {
                if (info.isLoaded()) {
                    ...
                    ModuleOperationResult result = moduleService.reload(className);
                    sendReloadResult(dto, messages, result);
                    any = true;
                }
            }
